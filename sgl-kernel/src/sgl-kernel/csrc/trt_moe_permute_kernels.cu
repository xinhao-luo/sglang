// Adapted from https://github.com/NVIDIA/TensorRT-LLM/blob/main/cpp/tensorrt_llm/kernels/mixtureOfExperts/moe_kernels.cu
    ...
    torch::Tensor token_expert_indices)        // [num_tokens, topk]
{
    ...
    int64_t workspace_size_bytes = (int64_t) get_workspace_size_for_radix_sort(
    ...
    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(
        input_tokens.scalar_type(), scalar_t, [&] {
            expandInputRowsKernelLauncher(
                static_cast<scalar_t*>(permuted_tokens.data_ptr()),
                reverse_permutation_map.data_ptr<int>(),
                static_cast<scalar_t*>(input_tokens.data_ptr()),
                sorted_token_expert_indices.data_ptr<int>(),
                num_tokens,
                hidden_size,
                topk,
